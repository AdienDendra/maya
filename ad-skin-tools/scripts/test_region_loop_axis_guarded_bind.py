"""v3.10H visual bind for loop-axis guarded Region consensus."""
...
from ad_skin_tools.region import loop_axis_guarded_consensus
...
for module in (loop_axis_guarded_consensus, region_solver):
...
def _loaded_unskinned_context():
    ...
    if tool_window is None:
        raise RuntimeError(
            "Open AD Skin Tool before running the v3.10H visual smoke test."
        )
...
def _validate_stored_weights(adapter, expected_weights, expected_columns):
    ...
    if maximum_difference > STORED_WEIGHT_TOLERANCE:
        raise RuntimeError(
            "Maya stored weights differ from v3.10H. Maximum difference: {}".format(
                maximum_difference
            )
        )
    ...
    if not np.array_equal(actual_columns, expected_columns):
        ...
        raise RuntimeError(
            "Stored hard owners differ from v3.10H. First IDs: {}".format(
                bad.tolist()
            )
        )
...
def _print_two_owner_diagnostics(region_result, result):
    relevant = [
        ...
        in {
            loop_axis_guarded_consensus.AXIAL_TWO_OWNER,
            loop_axis_guarded_consensus.NON_AXIAL_TWO_OWNER,
            loop_axis_guarded_consensus.DEGENERATE_TWO_OWNER,
            loop_axis_guarded_consensus.EXACT_COST_TIE,
        }
    ]
    ...
    for diagnostic in relevant[:PRINT_LOOP_LIMIT]:
        ...
        print(
            "  loop {} | {} | owners={} | counts={} | alignment={:.8f} | "
            "normal²={:.8f} | plane²={:.8f} | action={}".format(
                diagnostic.loop_index,
                diagnostic.classification,
                owners,
                list(diagnostic.owner_counts),
                diagnostic.axis_alignment,
                diagnostic.normal_component_squared,
                diagnostic.plane_component_squared,
                winner,
            )
        )
...
def run():
    ...
    guarded_result = (
        loop_axis_guarded_consensus.solve_loop_axis_guarded_consensus(
            region_result
        )
    )
    ...
    adapter = None
    try:
        with undo_chunk("AD Skin Tool v3.10H Loop Axis Guarded Bind"):
    ...
    builtins.AD_SKIN_V310H_REGION_RESULT = region_result
    builtins.AD_SKIN_V310H_GUARDED_RESULT = guarded_result
    builtins.AD_SKIN_V310H_SKIN_CLUSTER = adapter.skin_cluster

    cmds.select(region_result.mesh_transform, replace=True)

    print("\n[AD Skin Tool v3.10H - Loop Axis Guarded Visual Bind]")
    ...
    print(
        "Single-owner loops:",
        _count(guarded_result, loop_axis_guarded_consensus.SINGLE_OWNER),
    )
    print(
        "Axial two-owner loops:",
        _count(guarded_result, loop_axis_guarded_consensus.AXIAL_TWO_OWNER),
    )
    print(
        "Non-axial two-owner loops preserved:",
        _count(
            guarded_result,
            loop_axis_guarded_consensus.NON_AXIAL_TWO_OWNER,
        ),
    ...
    print(
        "Degenerate two-owner loops preserved:",
        _count(
            guarded_result,
            loop_axis_guarded_consensus.DEGENERATE_TWO_OWNER,
        ),
    ...
    print(
        ...
        _count(guarded_result, loop_axis_guarded_consensus.EXACT_COST_TIE),
    )
    print(
        "Multi-owner loops ignored:",
        _count(guarded_result, loop_axis_guarded_consensus.MULTI_OWNER_IGNORED),
    )
    print("Applied axial loops:", guarded_result.applied_loop_count)
    ...
    print(
        "\nThe mesh is bound only with the v3.10H loop-axis guard. "
        "No post-loop facing or ambiguous-neighbour correction was applied."
    )
    print(
