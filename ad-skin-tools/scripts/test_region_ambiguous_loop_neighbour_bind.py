"""v3.10K visual bind: v3.10J plus ambiguous closed-loop neighbour assignment."""
...
from ad_skin_tools.region import ambiguous_loop_neighbour
...
def _print_ambiguous_diagnostics(region_result, result):
    ...
    for diagnostic in result.diagnostics:
        ...
        for support in diagnostic.neighbour_supports:
            neighbour = _short_name(
                region_result.influences[int(support.owner_index)]
            )
            print(
                "    neighbour={} | boundary edges={} | loop-supported edges={} | "
                "closed loops={}".format(
                    neighbour,
                    support.boundary_edge_count,
                    support.loop_supported_edge_count,
                    list(support.supporting_loop_indices),
                )
            )
...
def run():
    ...
    neighbour_result = (
        ambiguous_loop_neighbour.solve_ambiguous_loop_neighbours(
            region_result,
            guarded_result,
        )
    )
    ...
    adapter = None
    try:
        with undo_chunk("AD Skin Tool v3.10K Ambiguous Loop Neighbour Bind"):
            ...
            vertex_ids, weights, expected_columns = _weights_in_skin_order(
                adapter,
                region_result,
                neighbour_result.corrected_owner_indices,
            )
    ...
    builtins.AD_SKIN_V310K_NEIGHBOUR_RESULT = neighbour_result
    builtins.AD_SKIN_V310K_SKIN_CLUSTER = adapter.skin_cluster

    cmds.select(region_result.mesh_transform, replace=True)

    print("\n[AD Skin Tool v3.10K - Ambiguous Loop Neighbour Visual Bind]")
    ...
    print("Ambiguous islands before assignment:", neighbour_result.ambiguous_region_count)
    print("Assigned ambiguous islands:", neighbour_result.assigned_region_count)
    print("Vertices assigned by loop neighbour:", neighbour_result.changed_vertex_count)
    print(
        "Final detached vertices:",
        neighbour_result.final_validation.detached_vertex_count,
    )
    print(
        "Final ambiguous vertices:",
        neighbour_result.final_validation.ambiguous_vertex_count,
    )
    if neighbour_result.final_validation.ambiguous_vertex_ids:
        print(
            "  final ambiguous IDs:",
            list(neighbour_result.final_validation.ambiguous_vertex_ids),
        )
    print("SkinCluster:", adapter.skin_cluster)
    print("Stored maximum weight difference:", maximum_difference)

    _print_ambiguous_diagnostics(region_result, neighbour_result)

    print(
        "\nThe mesh is bound with v3.10J plus one ambiguous-island rule: "
        "a unique neighbouring owner with the strongest single-owner closed-loop "
        "boundary support receives the whole island."
    )
    print(
