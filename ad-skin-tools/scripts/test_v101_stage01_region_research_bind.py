...
This is intentionally separate from the production Bind Skin button. It reads the
currently loaded unskinned mesh and the complete flat joint list from AD Skin Tool,
runs Region Research Stage 01, creates a temporary real skinCluster result, and
writes one hard 1.0 owner per vertex so the result can be inspected directly in
Maya.
...
# Reload only the new research path used by this smoke test. Production Bind Skin
# modules and UI callbacks are deliberately not replaced.
importlib.reload(region_runner)
