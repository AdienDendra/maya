"""Focused first-surface visibility smoke test for AD Skin Tool v3.

Required state:
    1. Run test_v30_distance_ranking.py first.
    2. Select exactly one joint from that Stage-1A result.
...
if not hasattr(builtins, "AD_SKIN_V30_DISTANCE_RESULT"):
    raise RuntimeError(
        "Run test_v30_distance_ranking.py before the visibility probe."
    )
...
builtins.AD_SKIN_V30_VISIBILITY_RESULT = result

short_source = result.source_joint.split("|")[-1]
builtins.print(
    "\n[AD Skin Tool v3.0 - Focused First-Surface Visibility Probe]"
)
...
builtins.print(
    "\nSaved result as builtins.AD_SKIN_V30_VISIBILITY_RESULT"
)
builtins.print(
    "Selection helpers:\n"
    "from ad_skin_tools.v3.visibility_probe import select_probe_vertices\n"
    "select_probe_vertices(builtins.AD_SKIN_V30_VISIBILITY_RESULT, "
    "category='visible')\n"
    "select_probe_vertices(builtins.AD_SKIN_V30_VISIBILITY_RESULT, "
    "category='rejected')"
)
