...
MINIMUM_COMPONENT_BLEND = baseline_smooth.MINIMUM_COMPONENT_BLEND
MAXIMUM_COMPONENT_BLEND = baseline_smooth.MAXIMUM_COMPONENT_BLEND
DEFAULT_COMPONENT_BLEND = baseline_smooth.DEFAULT_COMPONENT_BLEND
MINIMUM_COMPONENT_ITERATIONS = baseline_smooth.MINIMUM_COMPONENT_ITERATIONS
MAXIMUM_COMPONENT_ITERATIONS = baseline_smooth.MAXIMUM_COMPONENT_ITERATIONS
DEFAULT_COMPONENT_ITERATIONS = baseline_smooth.DEFAULT_COMPONENT_ITERATIONS
...
ComponentSmoothScope = baseline_smooth.ComponentSmoothScope
collect_smooth_scope = baseline_smooth.collect_smooth_scope
...
def print_component_smooth_report(result: ComponentSmoothResult) -> None:
    print("\n[AD Skin Tool Component Smooth Shared]")
    print("Kernel: bind_smoothing.diffusion")
    ...
    print("Context vertices:", result.context_vertex_count)
    ...
    print("Adjacency + mapping:", round(result.adjacency_seconds, 6))
    print("Weight read:", round(result.weight_read_seconds, 6))
    print("Shared calculation:", round(result.calculation_seconds, 6))
    print("  Shared topology:", round(result.shared_topology_seconds, 6))
    print("  Shared iterations:", round(result.shared_iteration_seconds, 6))
    print("  Shared finalization:", round(result.shared_finalization_seconds, 6))
    print("Undoable weight write:", round(result.weight_write_seconds, 6))
    print("Readback validation:", round(result.validation_seconds, 6))
...
def _smooth_context_rows(
    ...
    """Compatibility result matching the baseline private helper."""

    result = _solve_context_rows(
        baseline=baseline,
        selected_context_rows=selected_context_rows,
        selected_vertex_ids=selected_vertex_ids,
        selected_neighbour_rows=selected_neighbour_rows,
    ...
    return (
        ...
        result.skipped_empty_vertex_ids,
        result.skipped_locked_vertex_ids,
    )
