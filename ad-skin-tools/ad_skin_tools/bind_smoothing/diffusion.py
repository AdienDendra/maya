"""Topology-only diffusion for the AD Skin Tool bind-smoothing pipeline.

This module is deliberately independent from Maya scene commands. It accepts
hard Region owner indices plus mesh adjacency and returns an in-memory dense
weight matrix.

The v6.0 first smoke stage intentionally does NOT perform:
- maximum-influence pruning;
- locked-influence constraints;
- Region-state persistence;
- skinCluster writes.

Those stages are added only after the raw diffusion behaviour is verified.
"""
...
def diffuse_hard_ownership(
    ...
    """Diffuse one-hot Region ownership through connected polygon edges.

    Every iteration is Jacobi-style: all new rows are calculated from the
    previous complete matrix. Vertex order therefore cannot affect the result.

    ``relaxation`` controls how far one iteration moves toward the average
    weight row of connected neighbours:
    ...
    mixed_vertex_ids = np.where(
        active_counts > 1
    )[0].astype(np.int32)

    dominant_owner_changed_vertex_ids = np.where(
        np.argmax(current, axis=1).astype(np.int32)
        != owners
    )[0].astype(np.int32)
    ...
    return BindDiffusionResult(
        ...
        changed_vertex_ids=tuple(
            int(value)
            for value in changed_vertex_ids.tolist()
        ),
        mixed_vertex_ids=tuple(
            int(value)
            for value in mixed_vertex_ids.tolist()
        ),
...
def _validate_inputs(
    ...
    if owners.ndim != 1:
        raise ValueError(
            "owner_indices must be a one-dimensional array."
        )
    ...
    if owners.size:
        ...
        if np.any(invalid_owner_mask):
            ...
            raise ValueError(
                "owner_indices contains values outside the influence "
                "range. First vertex IDs: {}".format(
                    invalid_rows.tolist()
                )
            )
    ...
    for vertex_id, neighbours in enumerate(adjacency):
        for neighbour_id in neighbours:
            ...
            if neighbour_id == vertex_id:
                raise ValueError(
                    "Adjacency must not contain self-edges. "
                    "Vertex: {}.".format(vertex_id)
                )

    return owners


def _build_one_hot_weights(
    owner_indices,
    influence_count,
):
...
def _build_edge_arrays(adjacency):
    ...
    if (
        source_ids.size != edge_count
        or neighbour_ids.size != edge_count
    ):
        raise RuntimeError(
            "Failed to flatten mesh adjacency consistently."
        )
...
def _normalize_rows(
    ...
    if not np.all(np.isfinite(weights)):
        raise RuntimeError(
            "Bind diffusion produced non-finite weights."
        )
    ...
    if np.any(weights < -tolerance):
        ...
        raise RuntimeError(
            "Bind diffusion produced negative weights. "
    ...
    if invalid_rows.size:
        raise RuntimeError(
            "Bind diffusion produced empty weight rows. "
            "First vertex IDs: {}".format(
