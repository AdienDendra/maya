...
def enforce_maximum_influences(
    ...
    The hard blocking owner is never removed. Equal-weight cutoff ties are
    reported; v7 production uses the distance-aware constraint module instead.
    """
    ...
    near_zero_mask = (projected > 0.0) & (projected <= weight_epsilon)
    ...
    for vertex_id in range(vertex_count):
        ...
        if owner_column not in active_columns:
            active_columns = np.append(active_columns, owner_column)
        if active_columns.size <= maximum_influences:
            continue

        pruned_vertex_ids.append(vertex_id)
        ranked_columns = np.lexsort((all_columns, -row)).astype(np.int32)
    ...
    projected = _normalize_rows(projected, owners, weight_epsilon)
    ...
    return MaxInfluenceProjectionResult(
        ...
        pruned_vertex_ids=tuple(int(value) for value in pruned_vertex_ids),
...
def _validate_inputs(weights, owner_indices, maximum_influences, weight_epsilon):
    ...
    if owners.shape != (matrix.shape[0],):
        raise ValueError("owner_indices must contain one value per weight row.")
    if matrix.shape[1] < 1:
        raise ValueError("weights must contain at least one influence column.")
    ...
    if owners.size:
        ...
        if np.any(invalid):
            raise ValueError("owner_indices contains invalid influence columns.")
    return matrix, owners, maximum_influences, weight_epsilon


def _normalize_rows(weights, owner_indices, weight_epsilon):
    row_sums = np.sum(weights, axis=1, dtype=np.float64)
    empty_rows = np.where(row_sums <= weight_epsilon)[0]
    if empty_rows.size:
        weights[empty_rows] = 0.0
        weights[empty_rows, owner_indices[empty_rows]] = 1.0
        row_sums[empty_rows] = 1.0
    return weights / row_sums[:, np.newaxis]


def _active_histogram(weights, weight_epsilon):
    active_counts = np.count_nonzero(weights > weight_epsilon, axis=1)
