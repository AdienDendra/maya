...
def validate_bind_weights(
    ...
    if not np.all(np.isfinite(matrix)):
        bad = np.where(~np.all(np.isfinite(matrix), axis=1))[0][:20]
        raise RuntimeError(
    ...
    allowed_error = max(numerical_tolerance, weight_epsilon)
    ...
    excessive = np.where(active_counts > maximum_influences)[0]
    if excessive.size:
        raise RuntimeError(
            "Final bind weights exceed Max Influences. "
            "First vertex IDs: {}".format(excessive[:20].tolist())
        )
    ...
    if require_exact_one_hot:
        invalid = (
            (active_counts != 1)
            | (np.abs(owner_values - 1.0) > numerical_tolerance)
        )
        if np.any(invalid):
            bad = np.where(invalid)[0][:20]
            raise RuntimeError(
                "Iteration zero must remain exact one-hot blocking weights. "
                "First vertex IDs: {}".format(bad.tolist())
            )
    ...
    return BindWeightValidationResult(
        ...
        dominant_owner_changed_vertex_ids=tuple(
            int(value) for value in dominant_owner_changed.tolist()
        ),
    )
