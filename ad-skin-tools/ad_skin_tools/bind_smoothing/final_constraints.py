"""Small v6.1B constraints kept separate from the verified v6.1 solver."""
...
def project_region_owner_to_maximum(
    ...
    """Make the Region owner maximal with the smallest equalising adjustment.

    If another influence is above the owner, the owner is pooled with only the
    influences that must be lowered. Their total weight is preserved and shared
    equally. No floor, boost percentage, or artist-tuned threshold is used.
    """
...
def enforce_maximum_influences_by_distance(
    ...
    """Limit active weights, resolving equal cutoff weights by joint distance.

    The Region owner always occupies one slot. Remaining slots use larger weight
    first, then smaller vertex-to-joint squared distance. If both values are
    exactly tied at the cutoff, the row is left untouched and reported rather
    than silently falling back to influence order.
    """
    ...
    for vertex_id in range(projected.shape[0]):
        ...
        if slot_count > 0 and len(candidate_columns) > slot_count:
            ...
            same_weight = (
                float(row[selected_boundary])
                == float(row[excluded_boundary])
            )
            if same_weight:
...
def _owner_below_maximum_ids(matrix, owners, tolerance):
    ...
    return np.where(
        owner_values + tolerance < row_maximums
    )[0].astype(np.int32)
